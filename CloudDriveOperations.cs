    ...
    public static class CloudDriveOperations
    {
        ...
        public static CloudDriveListResponse<T> listSearch<T>(ConfigOperations.ConfigData config, String command)
        {
            ...
            do
            {
                ...
                while (String.IsNullOrWhiteSpace(mycontent))
                {
                    HttpClient request = createAuthenticatedClient(config, config.metaData.metadataUrl);
                    var task = request.GetStringAsync((String.IsNullOrWhiteSpace(nextToken) ? command : "nodes?startToken=" + nextToken));
                    task.Wait();
                    if (task.IsFaulted)
                        Console.WriteLine("Task faulted: {0} {1}", command, task.Exception.Message);
                    else if (task.IsCanceled)
                        Console.WriteLine("Task cancelled: {0} {1}", command, task.Result);
                    else
                        mycontent = task.Result;
                    Thread.Sleep(1000);
                }
        ...
        public static CloudDriveListResponse<CloudDriveFolder> getNewerFolders(ConfigOperations.ConfigData config, DateTime newerthan)
        {
            return listSearch<CloudDriveFolder>(config, "nodes?filters=kind:FOLDER AND modifiedDate:{" + newerthan.ToUniversalTime().ToString("o") + " TO *}");
        }

        public static CloudDriveListResponse<CloudDriveFile> getNewerFiles(ConfigOperations.ConfigData config, DateTime newerthan)
        {
            return listSearch<CloudDriveFile>(config, "nodes?filters=kind:FILE AND modifiedDate:{" + newerthan.ToUniversalTime().ToString("o") + " TO *}");
        }
